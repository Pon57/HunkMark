...
| `content/review-context.js` | Strict fingerprints, bounded aliases, and line-review context matching |
| `content/shared-completion.js` | Layout-invariant hunk completion sources, overlay application, and local-state restoration |
...
- A line key is independent of hunk boundaries and includes the displayed-review scope, file path, Unified/Split layout, change kind, exact content after line-ending normalization, file-wide occurrence, and total identical-line count. Its stored value carries a strict rendered-layout fingerprint derived from the contiguous changed block, its position in that block, adjacent rendered context, and a positional fallback when both context anchors are unavailable. Partial line state is intentionally local to one layout.
- Existing `hunkmark:v3` line entries without a layout discriminator remain valid only when their strict fingerprint matches the current DOM. Reconciliation then moves that entry to the matching layout-specific key under the storage lock; an entry first reopened in another layout remains unreviewed until its original layout validates the migration.
- The no-layout v3 compatibility path remains for at least one complete 180-day retention window after the first release with layout-specific keys. It may be removed only after every still-retainable entry has either been strictly migrated or expired; users may skip intermediate extension releases.
- Layout-invariant hunk signatures order deletions before additions while preserving order within each change kind, so an unbalanced Split replacement maps to GitHub's Unified ordering. Identical left/right context cells collapse to one semantic anchor. Distinct Split-side context text is ambiguous and receives no shared hunk key, leaving restoration fail-closed.
- A completed hunk stores one layout-invariant completion record with its Viewed and Collapse/Expand state. That atomic completion overlays layout-local line state in every exactly matching layout. A partial line change removes the overlay and restores each layout's own strict line state; an explicit whole-hunk Unviewed stores a canonical clear timestamp so older line state in every layout stays suppressed until that layout records newer review work. Partial line marks never cross layouts.
...
- An authorized migration may retain at most one validated contracted baseline beside the current expanded fingerprint. Reload, hidden-file restoration, and cross-tab adoption require an exact primary or baseline fingerprint plus the complete changed-line and context-anchor sequence; semantic replacement starts a new baseline instead of reviving a stale one. Shared hunk completion does not migrate across changed hunk boundaries.
